...
TEST_F(VisitorTest, DragonVsKnight) {
    ...
    EXPECT_FALSE(result); //Дракон не убивает рыцаря
    ...
    EXPECT_TRUE(result); //Рыцарь убивает дракона
}
...
TEST_F(VisitorTest, DragonVsPegasus) {
    ...
    EXPECT_TRUE(result); //Дракон ест пегаса
    ...
    EXPECT_FALSE(result); //Пегас никого не трогает
}
...
TEST_F(VisitorTest, KnightVsPegasus) {
    ...
    EXPECT_FALSE(result); //Рыцарь не трогает пегаса
...
TEST_F(VisitorTest, SameTypeFights) {
    ...
    EXPECT_FALSE(result); //Драконы не дерутся между собой
    ...
    EXPECT_FALSE(result); //Рыцари не дерутся между собой
    ...
    EXPECT_FALSE(result); //Пегасы не дерутся между собой
}
...
TEST_F(VisitorTest, AcceptMethod) {
    ...
    EXPECT_TRUE(result); //Рыцарь должен убить дракона
    
    //Дракон атакует пегаса
    result = pegasus->accept(visitor, dragon);
    EXPECT_TRUE(result); //Дракон должен съесть пегаса
